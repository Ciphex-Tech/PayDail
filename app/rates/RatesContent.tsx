...
export default function RatesContent({ initialMarkets, initialUpdatedAtIso, initialNairaRates }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"rates" | "calculator" | "crypto_prices" | "crypto_to_naira">("rates");
  ...
  const mobileRateRows = useMemo(() => {
    return exchangeCards.map((c) => {
  ...
  return (
    <div className="relative px-6 pt-3 sm:py-6">
      <TopToast
        open={toastOpen}
      ...
      <div className="lg:hidden -mx-6">
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-[18px] font-medium">Rates</h2>
              <p className="mt-1 text-[10px] sm:text-[12px] font-semibold text-[#9597A3]">Last updated : {updatedAt.toLocaleString()}</p>
            </div>
            <RefreshButton onClick={refresh} isRefreshing={isRefreshing} />
          </div>

          <div className="mt-4 grid grid-cols-4 bg-[#16161E] border border-[#2E2E3A] rounded-[12px] overflow-hidden">
            <button
              type="button"
              onClick={() => setActiveTab("rates")}
              className={`py-3 text-[11px] font-medium ${activeTab === "rates" ? "text-white bg-[#1D78FF]" : "text-white/60"}`}
            >
              Rates
            </button>
            <button
              ...
              className={`py-3 text-[11px] font-medium ${activeTab === "calculator" ? "text-white bg-[#1D78FF]" : "text-white/60"}`}
            >
              Calculator
            </button>
            <button
              type="button"
              onClick={() => setActiveTab("crypto_prices")}
              className={`py-3 text-[11px] font-medium ${activeTab === "crypto_prices" ? "text-white bg-[#1D78FF]" : "text-white/60"}`}
            >
              Prices
            </button>
            <button
              type="button"
              onClick={() => setActiveTab("crypto_to_naira")}
              className={`py-3 text-[11px] font-medium ${activeTab === "crypto_to_naira" ? "text-white bg-[#1D78FF]" : "text-white/60"}`}
            >
              To Naira
            </button>
          </div>
        ...
        {activeTab === "rates" ? (
          <div className="px-6 pb-6">
            <div className="space-y-3">
              {mobileRateRows.map((r) => (
                <div
                  key={r.sym}
                  className="flex items-center justify-between rounded-[12px] border border-[#2E2E3A] bg-[#16161E] px-4 py-3"
                >
                  <div className="flex items-center gap-3">
                    {r.icon ? (
                      <div className="relative h-[20px] w-[20px] overflow-hidden rounded-full">
                        <Image src={r.icon} alt="" fill sizes="20px" />
                      </div>
                    ) : (
                      <div className="h-[20px] w-[20px] rounded-full bg-white/10 flex items-center justify-center text-[10px] font-semibold">
                        {r.sym[0]}
                      </div>
                    )}
                    <span className="text-[13px] font-semibold text-white">{r.sym}</span>
                  </div>

                  <span className="text-[13px] font-medium text-white/90">$1 = ₦{Number(r.rate).toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === "calculator" ? (
          <div className="px-6 pb-6">
            <section className="rounded-[12px] bg-[#16161E] border border-[#2E2E3A] p-5">
              ...
              <div className="mt-6">
                <p className="text-[12px] sm:text-[14px] text-white">Select Cryptocurrency</p>

                <div className="relative mt-3">
                  <button
                    type="button"
                    onClick={() => setCalcOpen((v) => !v)}
                    className="flex h-[40px] sm:h-[48px] w-full items-center justify-between rounded-[12px] bg-[#20202C] border border-[#2E2E3A] px-4 text-[14px]"
                  >
                    <span className="flex items-center gap-2">
                      {calcMarket?.image ? (
                        ...
                        <span className="w-[17px] h-[17px] sm:h-[20px] sm:w-[20px] rounded-full bg-white/10 flex items-center justify-center text-[10px] font-semibold">
                          {calcSymbol[0]}
                        </span>
                      )}
                      <span className="text-[12px] sm:text-[14px]">{calcSymbol}</span>
                    </span>
                      ...
                      {exchangeCards.map((c) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex w-full items-center gap-2 px-4 py-3 text-left text-[12px] sm:text-[14px] ${
                          ...
                          >
                            ...
                            <span className="text-[12px] sm:text-[14px]">{c.sym}</span>
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="mt-6">
                <p className="text-[12px] sm:text-[14px] text-white">Enter Amount</p>
                <input
                  value={calcAmount}
                  onChange={(e) => setCalcAmount(e.target.value)}
                  className="mt-3 h-[40px] sm:h-[48px] w-full rounded-[12px] bg-[#20202C] border border-[#2E2E3A] px-4 text-[14px]"
                  placeholder="100"
                />
              </div>
              ...
              <button
                ...
                className="mt-6 w-full rounded-[12px] bg-[#1D78FF] py-3 text-[12px] sm:text-[16px] font-medium"
              >
                {isCalculating ? "Calculating..." : "Calculate"}
              </button>

              <div className="mt-6 sm:flex justify-end hidden">
      ...
      <div className="hidden lg:grid mt-6 gap-4 lg:grid-cols-4">
        {exchangeCards.map((c) => {
          const market = markets[c.sym as keyof typeof markets];
      ...
      <div className="hidden lg:grid mt-8 gap-6 lg:grid-cols-3">
        <section className="rounded-[12px] bg-[#16161E] border border-[#2E2E3A] p-5">
          <div className="flex items-center gap-3">
            <Image src="/images/calculator.svg" alt="" width={14} height={14} />
            <h3 className="text-[16px] font-medium">Rate Calculator</h3>
      ...
      {quoteOpen ? (
        <div
          className="fixed inset-0 z-50 flex items-center sm:items-center justify-center bg-[#0B0A0FCC] px-4"
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setQuoteOpen(false);
          }}
        >
          <div className="w-full max-w-[420px] max-h-[calc(100vh-32px)] overflow-y-auto rounded-t-[18px] sm:rounded-[18px] border border-[#2E2E3A] bg-[#16161E] p-4 sm:p-6">
            <div className="flex items-start justify-between">
              <div>
            ...
            <div className="mt-4 sm:mt-8 rounded-[12px] bg-[#20202C] border border-[#2E2E3A] p-4 text-[12px] text-[#A0A3AC]">
            ...
            <div className="mt-6 h-px w-full bg-[#2E2E3A] hidden sm:block" />
            ...
            <button
              ...
              className="mt-3 sm:mt-6 w-full rounded-[12px] bg-[#3B82F6] py-3 text-[14px] sm:text-[16px] font-medium"
            >
              Deposit Now
            </button>

            <div className="mt-3 sm:mt-6 rounded-[12px] bg-[#20202C] border border-[#2E2E3A] p-3 sm:p-4 text-[10px] sm:text-[12px] text-[#A0A3AC]">
